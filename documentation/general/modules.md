...
options. But contrary to the \myreflink{Monolithic} usage, the one letter *GMT* option syntax may be
...
[`-B documentation`](http://docs.generic-mapping-tools.org/latest/gmt.html#b-full).
...
option are explained in [`-R documentation`](http://docs.generic-mapping-tools.org/latest/gmt.html#r-full).
...
(see more at [`GMT defaults`](https://docs.generic-mapping-tools.org/latest/GMT_Docs.html#gmt-defaults)). 
...
follows closely the syntax described at [`gmt.conf`](https://docs.generic-mapping-tools.org/latest/gmt.conf.html)
...
keywords. See the full doc at [`-J documentation`](http://docs.generic-mapping-tools.org/latest/gmt.html#j-full). 
...
As referred in the \myreflink{Monolithic} section, we have two programs to do read and writing. Their
