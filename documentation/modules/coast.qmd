...
{{< include ../common_opts/explain_Acoast.qmd >}}

{{< include ../common_opts/opt_B.qmd >}}

- **C** or **riverfill** or **river\_fill** : -- *river\_fill=fill* **|** *river\_fill=(lake=true, river\_lake=true, fill=fill)*\
...
- **E** or **DCW** : -- *DCW=code1,code2,...* **|** *DCW=(country=code, continent=code, states=code, pen=pen, fill=fill, file=fname, inside=true, outside=false, adjust_r=??, , adjust_R=??, , adjust_e=??, headers=false)*\
   ...
   *country* or **name=code(s)**, where **code(s)** is a one or more comma-separated countries using the 2-character
   ...
   or SA (South America). Use **pen=pen** (see [Pen attributes]) to draw polygon outlines and **fill=fill**
   (see [Fill color/pattern]) to fill them [default is no fill]. At least one of these must be specified
   ...
   **dump** are set then we just print the **region**. The **file=fname** is a option to let users select alternative
   ...
   the region boundaries adding the amounts specified by _inc_ or _xinc/yinc_ or _winc/einc/sinc/ninc_.
...
{{< include ../common_opts/opt_box.qmd >}}
...
{{< include ../common_opts/opt_map_scale.qmd >}}
...
- **minpts** : -- *minpts=??*\
   ...
   This option is only available when used together with **E** or **DCW**. Useful when the country polygons are used in {{< gmtref zonal_stats >}} and you want to get rid of the small polygons that only consume time.
...
{{< include ../common_opts/opt_rose.qmd >}}

{{< include ../common_opts/opt_compass.qmd >}}
...
- **W** or **shore** or **shorelines** or **coast** or **coastlines** : -- *shore=pen* **|** *shore=(level=lev, pen=pen)*\
   Draw shorelines [the Default]. Append pen attributes (see [Pen attributes]) [Defaults: width = default,
...
- **Z** : -- *Z=true* **|** *Z=grid*\
   ...
   with zeros when **Z=true** or it interpolates the grid _grid_ (a file name or a {{< gmtref GMTgrid type=gmttype >}} object) with
   {{< gmtref grdrack >}} at the locations of the coastline. This is particularly useful if one wants to drape a coastline
   on top of a {{< gmtref grdview >}} plot.
...
{{< include ../common_opts/explain_gshhg.qmd >}}
```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("coast")
```
