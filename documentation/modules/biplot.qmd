...
- `cmd0`: The name of a 2D data table file readable with ``gmtread()``, or a `{{< gmtref GMTdataset type=gmttype >}}` with the data.
...
- `varlabels`: whether to plot variable labels. The default is :yes and it prints as labels the column names
   if the input {{< gmtref GMTdataset type=gmttype >}}. Is it hasn't column names, then it prints "A,B,C,...". Give a string vector to plot custom labels.
