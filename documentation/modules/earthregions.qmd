...
- `exact`: The region boundaries in the collections were rounded to more friendly numbers (few decimals).
    This means that they differ slightly from the pure ``GMT`` ({{< gmtref plot >}}) numbers. Setting `exact=true` will
    force using the strict ``GMT`` limits.

{{< include ../common_opts/opt_save_fig.qmd >}}

-----------
This module is a subset of {{< gmtref coast >}} but uses also calls to {{< gmtref grdcut >}}, {{< gmtref plot >}}
...
#| output: asis
using GMT
GMT.doc_source_links("earthregions")
```
