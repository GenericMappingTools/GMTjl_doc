...
Plots gray scales or color scales on maps. Both horizontal and vertical scales are supported. For CPTs with
gradational colors (i.e., the lower and upper boundary of an interval have different colors) we will interpolate
to give a continuous scale. Variations in intensity due to shading/illumination may be displayed by setting the
option **shade**. Colors may be spaced according to a linear scale, all be equal size, or by providing a file
with individual tile widths. The font used for the annotations along the scale and optional units is specified
by [FONT_ANNOT_PRIMARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_ANNOT_PRIMARY).
If a label is requested, it is plotted with [FONT_LABEL](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_LABEL).
...
- **D** or **pos** or **position** : -- *pos=(map=true, inside=true, outside=true, norm=true, paper=true, anchor=XX, size=XX, triangles=true, justify=code, offset=XX, horizontal=true, move\_annot=true[:a|:l|:u], neon=true, nan=true)*\
    ...
    _Note_: Given the number of options here, a convenient shortcut is provided _when embeding colorbar in other modules_.
    We can then, for examle, do `viz(..., colorbar=(xlabel="Xlabel", triangles=true|TopTriHigh))`. The value of `TopTriHigh`
    is a combination of `Top|Bot|Left|Right`, `Triangles` (or `Tri`) and `Low|high`. The first determines where the colorbar
    is located in respect to main figure. The second tells to add triangles at extremeties, and last choses if only one of
    the extremities gets a triangle. All are case insensitive. Details: Triangle indicators are only added if a CPT is currently
    loaded. It then changes the colormap background and foreground colors to match low and high colors. However, it is enphasized
    that the correct way of doing this is to set the bg and fg colors at the time of creating the colormap.

Optional Arguments
------------------

- **B** or **axes** or **frame**\
    Set annotation, tick, and gridline interval for the colorbar. More at [frame]
