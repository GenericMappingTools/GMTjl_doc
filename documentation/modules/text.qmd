...
Note that [PS_CHAR_ENCODING](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PS_CHAR_ENCODING) must be set to an extended character set in your `gmt.conf` file in order to use
...
- **textrecord**\
    A {{< gmtref GMTdataset type=gmttype >}} type that can be obtained with the `mat2ds(mat, txt)` function, where `mat` is a *MxN* with the
...
- **C** or **clearance** : -- *clearance=true* **|** *clearance=(margin=(dx,dy), round=true, concave=true, convex=true)*\
    ...
    **pen** or **fill** are specified. Append the unit you want (**c**m, **i**nch,
    or **p**oint); if not given we consult [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT)) or % for a percentage
...
- **F** or **attrib** : -- *attrib=(angle=val, font=font, justify=code, region\_justify=code, header=true, label=true, rec\_number=first, text=text, zvalues=format)*\
    ...
    ([FONT_ANNOT_PRIMARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_ANNOT_PRIMARY)), and centered on the data point. Use this option to override these
    ...
    **region** string and plots the text at the Upper Left corner of the map. Normally,the text
    ...
    supplied *format* (**zvalues=""** uses [FORMAT_FLOAT_MAP](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_FLOAT_MAP)). Note: If **threeD** is in effect then
...
For automatic non-overlapping label placement (similar to R's `ggrepel`), see the
[Text Repel examples](../../examples/plotting_functions/textrepel.qmd).

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("text")
```
