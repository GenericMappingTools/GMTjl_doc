...
Takes a Mx3 matrix, a {{< gmtref GMTdataset type=gmttype >}}, or a file name as input and returns either a table (a {{< gmtref GMTdataset type=gmttype >}}) or one or more
grids ({{< gmtref GMTgrid type=gmttype >}}). Aternatively, save the result directly in a disk file.
...
{{< include ../common_opts/opt_save_grd.qmd >}}
...
- **S** or **statistic** : -- *statistic=:m|:n|:s|:w*\
    ...
    in a table in a {{< gmtref GMTdataset type=gmttype >}} or in a grid. For this later case, however, one must use the **grid=true** option.
...
#| output: asis
using GMT
GMT.doc_source_links("blockmean")
```

See Also
--------

{{< gmtref blockmedian >}}, {{< gmtref blockmode >}}
