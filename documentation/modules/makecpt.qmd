...
and the NaN color (N) painted wherever values are undefined. For color tables beyond the current GMT offerings,
visit [cpt-city](https://phillips.shef.ac.uk/pub/cpt-city/).

If the master CPT includes B, F, and N entries, these will be copied into the new master file. If not, the parameters
[COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND),
[COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND),
and [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN) from the `gmt.conf`
file or the command line will be used. This default behavior can be overruled using the options **bg**, **overrule_bg** or **no_bg**.

The color model (RGB, HSV or CMYK) of the palette created by **makecpt** will be the same as specified in the header of
the master CPT. When there is no [COLOR_MODEL](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_MODEL)
entry in the master CPT, the [COLOR_MODEL](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_MODEL)
...
- **D** or **bg** or **background** : -- *bg=true* **|** *bg=:i*\
    ...
    [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND),
    [COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND),
    and [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN)].
    Append **i** to match the colors for the lowest and highest values in the input (instead of the output) CPT.
...
- **F** or **color_model** : -- *color_model=true|:r|:h|:c["+c"[label]]* **|** *color_model="+kkeys"*\
    ...
    will depend on the [FORMAT_TIME_PRIMARY_MAP](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_TIME_PRIMARY_MAP),
    [GMT_LANGUAGE](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-GMT_LANGUAGE)
    and possibly [TIME_WEEK_START](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-TIME_WEEK_START) settings.
...
- **I** or **inverse** or **reverse** : -- *inverse=true* **|** *inverse=:z*\
    ...
    including those specified by the parameters [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND) and
    [COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND).
    Use **inverse=:z** to reverse the sign of z-values in the color table. Note that this change of *z*-direction
    happens before **truncate** and **range** values are used so the latter much be compatible with the changed *z*-range.
    See also [Manipulating CPTs](https://docs.generic-mapping-tools.org/dev/reference/features.html#manipulating-cpts)
...
- **M** or **overrule_bg** : -- *overrule_bg=true*\
    ...
    [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND),
    [COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND), and
    [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN) specified in the
    `gmt.conf` file or on the command line. When combined with **bg**, only
    [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN) is considered.
...
{{< include ../common_opts/opt_range.qmd >}}
...
or simply use `gebco` directly in the application that needs the color table. To create a 24-level color table suitable
...
```{julia}
viz(makecpt(cmap=:gebco, reverse=:z), horizontal=true)
```
