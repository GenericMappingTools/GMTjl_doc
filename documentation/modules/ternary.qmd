...
- **L** or **vertex_labels** : -- *vertex_labels="Lab1/lab2/Lab3"* **|** *vertex_labels=("Lab1", "lab2", "Lab3")*\
    ...
    These are placed a distance of three times the [MAP_LABEL_OFFSET](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-MAP_LABEL_OFFSET)
...
- **S** or **marker** or **symbol** : -- *symbol=(symb=name, size=val, unit=unity)* or *marker|Marker|shape=name*, *markersize|MarkerSize|ms|size=val*\
    ...
    see the **symbol** option in {{< gmtref plot >}}

Other than the above options, the `kwargs` input accepts still the following options:

- **image** : -- *image=true*\
    Fills the ternary plot with an image computed automatically with {{< gmtref grdimage >}} from a grid
    interpolated with {{< gmtref surface >}}

- **contour** : -- *contour=??*\
    This option works in two different ways. If used together with `image` it overlays a contour
    by doing a call to {{< gmtref grdcontour >}}. However, if used alone it will call `contour` to do the contours.
...
- **contourf** : -- *contourf=??*\
    ...
    using automatic parameters. The form `contourf=(...)` let us selects options of the {{< gmtref contourf >}} module.
...
See other examples at [Ternary plots](https://www.generic-mapping-tools.org/GMTjl_doc/examples/ternary/ternary_examples.html)

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("ternary")
```
