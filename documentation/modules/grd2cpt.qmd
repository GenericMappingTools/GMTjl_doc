...
the colors are histogram equalized. Thus if the grid(s) and the resulting CPT are used in {{< gmtref grdimage >}}
...
tables beyond the current GMT offerings, visit [cpt-city](https://phillips.shef.ac.uk/pub/cpt-city/).

If the master CPT includes B, F, and N entries, these will be copied into the new master file.
If not, the parameters [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND),
[COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND),
and [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN) from the
...
The color model (RGB, HSV or CMYK) of the palette created by {{< gmtref makecpt >}} will be the same as
specified in the header of the master CPT. When there is no [COLOR_MODEL](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_MODEL) entry in the master CPT,
the [COLOR_MODEL](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_MODEL) specified in the `gmt.conf` (see {{< gmtref gmtset >}}) file or on the command line will be used.

Required Arguments
------------------

*ingrid* : -- A grid file name or a {{< gmtref GMTgrid type=gmttype >}}
...
{{< include ../common_opts/create_cpt.qmd >}}
...
- **D** or **bg** or **background** : -- *bg=true* **|** *bg=:i*\
    ...
    [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND), [COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND), and [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN)]. Append **i** to match the colors for the lowest
...
- **F** or **color_model** : -- *color_model=true|:r|:h|:c["+c"[label]]*\
    ...
    list of weekday names. The format of these labels will depend on the [FORMAT_TIME_PRIMARY_MAP](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_TIME_PRIMARY_MAP),
    [GMT_LANGUAGE](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-GMT_LANGUAGE) and possibly [TIME_WEEK_START](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-TIME_WEEK_START) settings.
...
- **I** or **inverse** or **reverse** : -- *inverse=true* **|** *inverse=:z*\
    ...
    including those specified by the parameters [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND) and [COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND). Use **inverse=:z** to
...
- **M** or **overrule_bg** : -- *overrule_bg=true*\
    ...
    [COLOR_BACKGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_BACKGROUND), [COLOR_FOREGROUND](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_FOREGROUND), and [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN) specified in the `gmt.conf` file or on the command line.
    When combined with **bg**, only [COLOR_NAN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-COLOR_NAN) is considered.
...
{{< include ../common_opts/opt_bo.qmd >}} This option only applies if **nlevels** selects CDF output. 

{{< include ../common_opts/opt_h.qmd >}}

{{< include ../common_opts/explain_transparency.qmd >}}
...
you cannot learn from {{< gmtref grdinfo >}}. So you can do this to see some points
...
{{< gmtref grdhisteq >}}, {{< gmtref grdinfo >}}, {{< gmtref makecpt >}}
