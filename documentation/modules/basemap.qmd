...
At least one of the options [frame], **map_scale**, or **rose** must be specified.
...
- **A** or **polygon** : *polygon=true* or *polygon=fname*\
    ...
    for the (possibly oblique) rectangular map domain. The plot domain must be given via [limits]
    and [proj], with no other options allowed. The sampling interval is controlled via [MAP_LINE_STEP](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-MAP_LINE_STEP)
    parameter. The coordinates are return in a {{< gmtref GMTdataset type=gmttype >}} or written to *fname* if a file
    name is specified.
...
The [proj] **G** option can be used in a more generalized form, specifying
...
must give the pole for the new projection and use the [limits] option to
...
The [frame] option sets up a regular annotation interval and the annotations derive
...
achieved by using **c** *intfile* in the [frame] option, where *intfile* contains all
...
The [frame] option is somewhat complicated to explain and comprehend.
