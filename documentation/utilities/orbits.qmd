...
- `first`: Boolean that indicates if ``orbits`` creates the first layer of the image plot. If the default
  ...
  by {{< gmtref grdimage >}} on a Eart's DEM. Note, however, that in this case the previous plot must have used the same
...
- `kw`: keyword arguments to be consumed in the {{< gmtref coast >}} and [plot3] calls. For example, `land=:tomato,
...
#| output: asis
using GMT
GMT.doc_source_links("orbits")
```

See Also
--------

{{< gmtref coast >}}, {{< gmtref plot3 >}}